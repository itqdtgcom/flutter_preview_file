    ...
    if (stat.size > config.maxTextFileSizeBytes) {
      ...
      try {
        ...
        return '${utf8.decode(bytes, allowMalformed: true)}\n\n'
            '[File truncated — first ${config.maxTextFileSizeBytes ~/ 1024} KB]';
      } finally {
        await raf.close();
      }
    }
    ...
    switch (node.type) {
      ...
      case NodeType.blockquote:
        return Container(
          ...
          child: Text(node.text, style: const TextStyle(fontStyle: FontStyle.italic)),
        );
          ...
          child: Row(
            children: [
              if (widget.keyName != null) ...[
                Text('"${widget.keyName}": ', style: TextStyle(color: _depthColor)),
              ],
              Icon(
...
class _CsvRendererState extends State<CsvRenderer> {
  ...
  Widget build(BuildContext context) {
    ...
    final headers = _rows!.isNotEmpty ? _rows![0] : <String>[];
      ...
      children: [
        ...
        if (dataRows.length > 500)
          Padding(
            ...
            child: Text(
              'Showing 500 of ${dataRows.length} rows',
...
class _CodeRendererState extends State<CodeRenderer> {
  ...
  Future<void> _load() async {
    try {
      ...
      final parts = widget.file.path.replaceAll('\\', '/').split('/');
      final name = parts.isNotEmpty ? parts.last : '';
      final dot = name.lastIndexOf('.');
      final ext = dot > 0 ? name.substring(dot + 1).toLowerCase() : '';
      final spans = SyntaxTokenizer.tokenize(code, ext, widget.config.syntaxTheme);
      if (mounted) {
        setState(() {
          _code = code;
          _spans = spans;
  ...
  @override
  Widget build(BuildContext context) {
    final theme = widget.config.syntaxTheme;
